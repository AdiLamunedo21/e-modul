            ...
            <div class="flex items-center gap-2.5 flex-wrap justify-between lg:justify-end">
                ...
                <form method="GET" action="{{ route('admin.library.index') }}" class="flex items-center gap-2 flex-wrap">
                    ...
                    <select name="sort"
                            ...
                            class="text-xs rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-slate-700 focus:bg-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all font-semibold">
                    ...
                    @if($search || ($subjectId && $subjectId !== 'all') || ($grade && $grade !== 'all') || ($teacherId && $teacherId !== 'all') || $sort !== 'popular')

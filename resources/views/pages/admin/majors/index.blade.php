...
@section('title', 'Master Jurusan & Keahlian — Admin E-Modul')
@section('page-title', 'Master Jurusan & Keahlian')
...
<div x-data="{
    ...
    activeMajor: { id: null, name: '', code: '', description: '' },
    openEdit(major) {
        this.activeMajor = JSON.parse(JSON.stringify(major));
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div>
            <nav class="flex items-center gap-2 text-xs text-slate-400 mb-1">
                ...
                <span>/</span>
                <span class="text-slate-700 font-semibold">Master Jurusan</span>
            </nav>
            <h1 class="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight flex items-center gap-2.5">
                <span>Master Jurusan & Keahlian</span>
                <span class="text-xs font-bold px-2.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200">
                    {{ $stats['total'] }} Jurusan
                </span>
            </h1>
            <p class="mt-1 text-sm text-slate-500">
                Kelola program/konsentrasi keahlian vokasi yang menaungi rombongan belajar kelas.
            </p>
        </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-200/80 shadow-sm p-5 sm:p-6 mb-6">
        <form method="GET" action="{{ route('admin.majors.index') }}" class="flex items-center justify-between gap-4">
            <div class="relative flex-1 max-w-md">
                ...
                <input type="text"
                       name="search"
                       value="{{ $search }}"
                       placeholder="Cari nama atau kode singkatan jurusan..."
            ...
            <div class="flex items-center gap-2">
                ...
                @if($search)
                    <a href="{{ route('admin.majors.index') }}" class="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl text-xs font-semibold transition-colors">
                        Reset
                    </a>
                @endif
            </div>
            ...
            <table class="w-full text-left text-sm">
                <thead>
                    <tr class="bg-slate-50/80 border-b border-slate-100 text-xs font-bold text-slate-400 uppercase tracking-wider">
                        <th class="py-4 px-6">Kode & Nama Jurusan</th>
                        <th class="py-4 px-4">Deskripsi / Ruang Lingkup</th>
                        <th class="py-4 px-4 text-center">Rombel Kelas</th>
                        <th class="py-4 px-6 text-right">Aksi Manajemen</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                    @forelse($majors as $maj)
                        @php
                            $majJson = json_encode([
                                'id' => $maj->id,
                                'name' => $maj->name,
                        ...
                        <tr class="hover:bg-slate-50/60 transition-colors">
                            ...
                            <td class="py-4 px-6">
                                <div class="flex items-center gap-3">
                                    <span class="inline-flex items-center px-3 py-1 rounded-xl text-xs font-black bg-indigo-50 text-indigo-700 border border-indigo-200 font-mono">
                                        {{ $maj->code }}
                                    </span>
                                    <div>
                                        <p class="font-bold text-slate-900 text-xs">{{ $maj->name }}</p>
                                    </div>
                                </div>
                            </td>

                            {{-- Deskripsi --}}
                            <td class="py-4 px-4 text-xs text-slate-500 max-w-sm">
                                <p class="line-clamp-2 leading-relaxed">{{ $maj->description ?: '-' }}</p>
                            </td>

                            {{-- Kelas Terdaftar --}}
                            <td class="py-4 px-4 text-center">
                                <a href="{{ route('admin.classes.index', ['major_id' => $maj->id]) }}"
                                   class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-slate-100 text-slate-700 hover:bg-indigo-50 hover:text-indigo-700 transition-colors border border-slate-200">
                                    {{ $maj->classes_count }} Rombel Kelas &rarr;
                                </a>
                            </td>
                ...
                <form action="{{ route('admin.majors.store') }}" method="POST">
                    ...
                    <div class="bg-white p-6 sm:p-7">
                        <div class="flex items-center justify-between pb-4 border-b border-slate-100 mb-5">
                            <h3 class="text-lg font-black text-slate-900">Tambah Jurusan Baru</h3>
                        ...
                        <div class="space-y-4 text-xs">
                            ...
                            <div>
                                ...
                                <textarea name="description" rows="3" placeholder="Uraian ringkas kompetensi keahlian jurusan..." class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"></textarea>
                            </div>
                        </div>
                    </div>
    ...
    <div x-cloak 
        ...
        <div class="flex min-h-screen items-center justify-center p-4 sm:p-6 lg:px-8 text-center">
            <div x-show="editModalOpen"
                ...
                <form :action="'{{ url('/admin/majors') }}/' + activeMajor.id" method="POST">
                    @csrf
                    @method('PATCH')

                    <div class="bg-white p-6 sm:p-7">
                        <div class="flex items-center justify-between pb-4 border-b border-slate-100 mb-5">
                            <h3 class="text-lg font-black text-slate-900">Edit Data Jurusan</h3>
                        ...
                        <div class="space-y-4 text-xs">
                            <div>
                                ...
                                <input type="text" name="name" x-model="activeMajor.name" required class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500">
                            </div>

                            <div>
                                <label class="block font-bold text-slate-700 mb-1">Kode Singkatan <span class="text-red-500">*</span></label>
                                <input type="text" name="code" x-model="activeMajor.code" required class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 uppercase font-mono">
                            </div>

                            <div>
                                <label class="block font-bold text-slate-700 mb-1">Deskripsi Jurusan</label>
                                <textarea name="description" x-model="activeMajor.description" rows="3" class="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"></textarea>
                            </div>
                        </div>
                    </div>
            ...
            <div x-show="deleteModalOpen"
                ...
                <form :action="'{{ url('/admin/majors') }}/' + activeMajor.id" method="POST">
                    ...
                    <div class="bg-white p-6 sm:p-7 text-center">
                        ...
                        <h3 class="text-base font-black text-slate-900 mb-2">Hapus Jurusan?</h3>
                        <p class="text-xs text-slate-500 leading-relaxed">
                            Apakah Anda yakin ingin menghapus jurusan <strong class="text-slate-800" x-text="activeMajor.name"></strong> (<span class="font-mono text-slate-700" x-text="activeMajor.code"></span>)?
                        </p>
                        <p class="text-[11px] text-red-500 mt-2 font-medium">
                            Perhatian: Rombel kelas di bawah jurusan ini akan ikut terhapus.
                        </p>
                    </div>

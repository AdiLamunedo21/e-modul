    ...
    <div class="rounded-3xl bg-white border border-teal-200/90 shadow-sm overflow-hidden" id="section-pre-test">
        ...
        <div class="p-6 sm:p-8">
            ...
            <div x-show="showRetakeForm || {{ $initialPreScore === null ? 'true' : 'false' }}">
                ...
                @else
                    @php $preQuestionsCount = $module->preTest->questions->count(); @endphp
                            ...
                            <div class="flex flex-wrap gap-2 pt-1">
                                @foreach($module->preTest->questions as $idx => $q)
                                    <button type="button"
                                            @click="jumpTo({{ $idx }})"
                                            :class="{
                                                'ring-2 ring-teal-500 ring-offset-2 border-teal-600 scale-105 font-black shadow-sm': currentQuestion === {{ $idx }},
                                                'bg-teal-600 text-white border-teal-600': isAnswered('{{ $q->id }}') && currentQuestion !== {{ $idx }},
                                                'bg-white text-slate-700 border-slate-200 hover:border-teal-300 hover:bg-teal-50/30': !isAnswered('{{ $q->id }}') && currentQuestion !== {{ $idx }},
                                                'bg-teal-600 text-white': isAnswered('{{ $q->id }}') && currentQuestion === {{ $idx }}
                                            }"
                            ...
                            @foreach($module->preTest->questions as $idx => $q)
                                <div x-show="currentQuestion === {{ $idx }}"
                                    ...
                                    <div class="flex items-center justify-between pb-3 border-b border-slate-200/80 gap-3">
                                        <div class="flex items-center gap-2">
                                            <span class="w-8 h-8 rounded-xl bg-teal-600 text-white text-xs font-black flex items-center justify-center shadow-xs">
                                                {{ $idx + 1 }}
                                            </span>
                                            <div>
                            ...
                            <div class="pt-2 flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-slate-200">
                                <div class="w-full sm:w-auto flex items-center gap-2">
                                    ...
                                    <button type="button"
                                            ...
                                            class="flex-1 sm:flex-initial px-5 py-2.5 rounded-xl bg-teal-600 hover:bg-teal-700 text-white font-bold text-xs shadow-xs transition flex items-center justify-center gap-1.5 cursor-pointer">
                                ...
                                <div class="w-full sm:w-auto flex items-center gap-2 justify-end">
                                    ...
                                    <button type="button"
                                            ...
                                            :class="currentQuestion === totalQuestions - 1 ? 'ring-4 ring-teal-500/20' : ''"
                                            class="w-full sm:w-auto px-6 py-2.5 rounded-xl bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-700 hover:to-emerald-700 text-white font-black text-xs shadow-md shadow-teal-600/25 transition flex items-center justify-center gap-2 cursor-pointer">
                                        <span>🏁</span>
                                        <span>{{ $initialPreScore !== null ? 'Kirim Jawaban Latihan' : 'Kirim Jawaban Pre-test' }}</span>
                                    </button>
                                </div>
                            </div>
                        </form>

...
@section('title', 'Mata Pelajaran ' . $class->full_name . ' — Laporan Nilai Excel')
...
<div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
    <div>
        <div class="flex items-center gap-2 text-xs font-bold text-slate-400 uppercase tracking-widest mb-1.5">
            ...
            <span>/</span>
            <span class="text-emerald-600">{{ $class->full_name }}</span>
        </div>
        ...
        <p class="mt-1.5 text-sm text-slate-500 max-w-3xl leading-relaxed">
            Jurusan: <strong class="text-slate-700">{{ $class->major ? $class->major->name : $class->major_name }}</strong> • Tingkat {{ $class->grade }} • Rombel {{ $class->section }}. Pilih mata pelajaran untuk membuka modul pembelajaran terkait.
        </p>
    </div>
...
<div class="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8">
    ...
    <div class="rounded-2xl bg-white p-5 border border-slate-200/80 shadow-sm flex items-center gap-4">
        ...
        <div>
            <p class="text-xs font-bold text-slate-400 uppercase tracking-wider">Modul Pembelajaran Anda</p>
            <div class="flex items-baseline gap-1 mt-0.5">
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    @forelse($subjects as $sub)
        <div class="group relative rounded-3xl bg-white border border-slate-200/80 hover:border-emerald-300 transition-all duration-300 flex flex-col justify-between overflow-hidden shadow-sm hover:shadow-md">
            ...
            <div class="p-6 flex-1 flex flex-col justify-between space-y-4">
                ...
                <div class="pt-3 border-t border-slate-100 space-y-2 text-xs">
                    <div class="flex items-center justify-between">
                        <span class="text-slate-500 font-medium">Modul Ajar Anda:</span>
                    ...
                    <div class="flex items-center justify-between">
                        <span class="text-slate-500 font-medium">Siswa Tuntas Dinilai:</span>
                        <span class="font-bold text-emerald-700">
                            {{ $sub->class_graded_count }} Siswa
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="p-4 pt-0">
                <a href="{{ route('teacher.reports.class.subject', [$class->id, $sub->id]) }}"
                   class="w-full py-2.5 px-4 rounded-xl text-xs font-bold bg-emerald-600 hover:bg-emerald-700 text-white flex items-center justify-center gap-2 transition-all shadow-sm shadow-emerald-600/25 group-hover:shadow-md">
                    <span>Buka Modul Pembelajaran ({{ $sub->class_modules_count }})</span>

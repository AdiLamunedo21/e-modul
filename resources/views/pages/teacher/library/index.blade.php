...
@php
    $currentTeacherId = (string) auth()->guard('teacher')->id();
    $modulesToSerialize = isset($allSharedModules) && $allSharedModules->isNotEmpty() ? $allSharedModules : $modules->getCollection();
    $modulesJson = $modulesToSerialize->map(function($m) use ($currentTeacherId) {
        $sections = $m->moduleSectionsSummary();
        return [
            'id' => $m->id,
            'title' => $m->title,
            'teacher_id' => (string) $m->teacher_id,
            'teacher_name' => $m->teacher->name ?? 'Guru Pendidik',
            'is_mine' => ((string) $m->teacher_id) === $currentTeacherId,
            'subject_id' => (string) $m->subject_id,
            'subject_name' => $m->subject?->name ?? 'Mapel',
            'subject_icon' => $m->subject?->icon ?? '📖',
            'subject_badge_classes' => $m->subject ? $m->subject->badgeClasses() : 'bg-slate-100 text-slate-700',
            'grade' => $m->schoolClass?->grade ?? '',
            'major_name' => $m->schoolClass?->major_name ?? '',
            'class_name' => $m->schoolClass ? ($m->schoolClass->grade . ' ' . $m->schoolClass->major_name) : 'Semua Kelas',
            'cloned_from_teacher' => $m->clonedFrom?->teacher?->name ?? null,
            ...
            'show_url' => route('teacher.library.show', $m),
        ];
    })->values()->toArray();
@endphp

<div x-data="{
    tab: '{{ $tab ?? 'all' }}', // 'all', 'others', 'my_shared'
    searchKeyword: '{{ addslashes(request('search', '')) }}',
    selectedGrade: '{{ addslashes(request('grade', '')) }}',
    selectedMajor: '{{ addslashes(request('major', '')) }}',
    selectedTeacher: '{{ addslashes(request('teacher_id', '')) }}',
    activeSort: '{{ addslashes($sort ?? 'latest') }}', // 'latest', 'popular', 'title_asc'
    items: {{ json_encode($modulesJson) }},
    get displayItems() {
        let list = [...this.items];

        // 1. Tab Filtering (Instan)
        if (this.tab === 'others') {
            list = list.filter(m => !m.is_mine);
        } else if (this.tab === 'my_shared') {
            list = list.filter(m => m.is_mine);
        }

        // 2. Dropdown Filter Tingkat
        if (this.selectedGrade !== '') {
            list = list.filter(m => String(m.grade) === String(this.selectedGrade));
        }

        // 3. Dropdown Filter Jurusan
        if (this.selectedMajor !== '') {
            list = list.filter(m => String(m.major_name) === String(this.selectedMajor));
        }

        // 4. Dropdown Filter Guru
        ...
        } else if (this.activeSort === 'title_asc') {
    ...
    get isFiltered() {
        return this.searchKeyword.trim() !== '' || this.selectedGrade !== '' || this.selectedMajor !== '' || this.selectedTeacher !== '' || this.activeSort !== 'latest' || this.tab !== 'all';
    }
}">
    ...
    {{-- ══ 3. TABS & FILTER BAR (INSTAN TANPA REFRESH) ══ --}}
    <div class="space-y-4 mb-8">
        {{-- Quick Tabs --}}
        <div class="flex flex-wrap items-center gap-2 border-b border-slate-200 pb-3">
            <button type="button"
                    @click="tab = 'all'"
                    :class="tab === 'all' ? 'bg-indigo-600 text-white shadow-sm shadow-indigo-600/20' : 'bg-white text-slate-600 hover:bg-slate-100 border border-slate-200'"
                ...
                <span class="px-1.5 py-0.5 rounded-full text-[10px]" :class="tab === 'all' ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-600'" x-text="items.length"></span>
            </button>

            <button type="button"
                    @click="tab = 'others'"
                    :class="tab === 'others' ? 'bg-indigo-600 text-white shadow-sm shadow-indigo-600/20' : 'bg-white text-slate-600 hover:bg-slate-100 border border-slate-200'"
                    class="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer">
                <span>👥 Karya Guru Lain</span>
                <span class="px-1.5 py-0.5 rounded-full text-[10px]" :class="tab === 'others' ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-600'" x-text="items.filter(m => !m.is_mine).length"></span>
            </button>

            <button type="button"
                    @click="tab = 'my_shared'"
                    :class="tab === 'my_shared' ? 'bg-indigo-600 text-white shadow-sm shadow-indigo-600/20' : 'bg-white text-slate-600 hover:bg-slate-100 border border-slate-200'"
                    class="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer">
                <span>⭐ Modul Saya yang Dibagikan</span>
                <span class="px-1.5 py-0.5 rounded-full text-[10px]" :class="tab === 'my_shared' ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-600'" x-text="items.filter(m => m.is_mine).length"></span>
            </button>
        </div>

        {{-- Filter & Search Bar (Sejajar di Desktop) --}}
        <div class="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200/80 shadow-sm space-y-3">
            <div class="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3">
                ...
                <div class="relative flex-1 min-w-[220px] flex items-center">
                    ...
                    <input type="text"
                           ...
                           class="w-full pl-10 pr-9 py-2.5 text-xs sm:text-sm bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all placeholder:text-slate-400 font-medium">
                ...
                <div class="flex flex-wrap items-center gap-2.5">
                    ...
                    <button type="button"
                            ...
                            x-cloak
                            @click="resetFilters()"
                            class="inline-flex items-center gap-1.5 px-3.5 py-2.5 text-xs font-bold text-slate-500 bg-slate-100 hover:bg-slate-200 hover:text-slate-700 rounded-xl transition-all cursor-pointer"
                            title="Reset Filter">
            ...
            <div class="flex items-center gap-1.5 flex-wrap pt-3 border-t border-slate-100 text-xs">
                ...
                <button type="button"
                        ...
                        :class="activeSort === 'latest' ? 'bg-indigo-600 text-white shadow-xs font-bold' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'"
                ...
                <button type="button"
                        ...
                        :class="activeSort === 'popular' ? 'bg-amber-600 text-white shadow-xs font-bold' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'"
                        class="px-3 py-1.5 rounded-xl text-xs font-semibold transition-all cursor-pointer">
                    🔥 Paling Banyak Dikloning
                </button>

                <button type="button"
                        @click="activeSort = 'title_asc'"
                        :class="activeSort === 'title_asc' ? 'bg-purple-600 text-white shadow-xs font-bold' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'"
                        class="px-3 py-1.5 rounded-xl text-xs font-semibold transition-all cursor-pointer">
                    🔤 Judul (A - Z)
                </button>
            </div>
    ...
    {{-- ══ 4. GRID DAFTAR MODUL LIBRARY (REAKTIF INSTAN TANPA RELOAD) ══ --}}
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mb-8">
        <template x-for="mod in displayItems" :key="'teacher-card-' + mod.id">
            <div class="bg-white rounded-3xl border border-slate-200/80 shadow-sm hover:shadow-md transition-all flex flex-col justify-between overflow-hidden group">
                ...
                <div class="p-6">
                    <div class="flex items-start justify-between gap-3 mb-3">
                        <div>
                            <h4 class="text-xs font-bold text-slate-800 leading-tight">
                                <span x-text="mod.teacher_name"></span>
                                <template x-if="mod.is_mine">
                                    <span class="ml-1 text-[10px] px-1.5 py-0.5 rounded-md bg-amber-100 text-amber-800 font-extrabold">Anda</span>
                                </template>
                            </h4>
                            <p class="text-[10px] text-slate-400">
                                Dibagikan <span x-text="mod.shared_at"></span>
                            </p>
                        </div>

                        {{-- Target Grade, Major & Subject Badge --}}
                        <div class="flex items-center gap-1.5 flex-wrap justify-end">
                            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-xl text-[11px] font-bold" :class="mod.subject_badge_classes">
                                <span x-text="mod.subject_icon"></span>
                                <span x-text="mod.subject_name"></span>
                            </span>
                            <span class="inline-flex items-center px-2.5 py-1 rounded-xl text-[11px] font-bold bg-slate-100 text-slate-700 border border-slate-200 shrink-0" x-text="mod.class_name"></span>
                        </div>
                    </div>
                    ...
                    <h3 class="text-base font-extrabold text-slate-900 group-hover:text-indigo-600 transition-colors leading-snug line-clamp-2 mt-2">
                        <a :href="mod.show_url" x-text="mod.title"></a>
                    </h3>

                    {{-- Cloned Origin Attribution if this was previously cloned --}}
                    <template x-if="mod.cloned_from_teacher">
                        <p class="text-[11px] text-slate-400 mt-1 flex items-center gap-1 truncate">
                            <span>🌱</span>
                            <span>Diadaptasi dari karya <strong x-text="mod.cloned_from_teacher"></strong></span>
                        </p>
                    </template>

                    {{-- Component Pills --}}
                    <div class="mt-4 flex flex-wrap gap-1.5">
                        <template x-if="mod.has_materi"><span class="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-blue-50 text-blue-700 border border-blue-200">📖 Materi</span></template>
                        <template x-if="mod.has_video"><span class="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-red-50 text-red-700 border border-red-200">🎬 Video</span></template>
                        <template x-if="mod.has_pre_test"><span class="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200">📝 Pre-test</span></template>
                        <template x-if="mod.has_post_test"><span class="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-teal-50 text-teal-700 border border-teal-200">🎯 Post-test</span></template>
                        <template x-if="mod.has_embed"><span class="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-purple-50 text-purple-700 border border-purple-200">🎮 Embed</span></template>
                        <template x-if="mod.has_job_sheet"><span class="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-amber-50 text-amber-700 border border-amber-200">📋 Job Sheet</span></template>
                        <template x-if="mod.has_lkpd"><span class="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-cyan-50 text-cyan-700 border border-cyan-200">📑 LKPD</span></template>
                    </div>
                </div>
                ...
                <div class="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between gap-3">
                    ...
                    <div class="flex items-center gap-1.5 text-xs text-slate-500 font-semibold" title="Jumlah guru yang telah menyalin modul ini">
                        ...
                        <span x-text="mod.clone_count + 'x Disalin'"></span>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        {{-- Trigger Clone Modal --}}
                        <button type="button"
                                @click="openCloneModal(mod.id, mod.title, mod.teacher_name)"
                                class="inline-flex items-center gap-1.5 px-3.5 py-2 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl shadow-sm shadow-indigo-600/25 transition-all cursor-pointer">
    ...
    {{-- Empty State in Alpine --}}
    <div x-show="displayItems.length === 0" class="bg-white rounded-3xl p-12 text-center border border-slate-200/80 shadow-sm max-w-2xl mx-auto my-6">
        ...
        <button type="button"
                @click="resetFilters()"
                class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs transition-all cursor-pointer">
    ...
    <div class="bg-white rounded-3xl max-w-lg w-full p-6 sm:p-8 shadow-2xl border border-slate-200 relative transform transition-all">
        ...
        <form id="cloneForm" method="POST" action="" class="space-y-4">
            ...
            <div>
                ...
                <div class="p-3 rounded-xl bg-slate-50 border border-slate-200 text-xs font-bold text-slate-800" id="cloneSourceTitle">
                    -
                </div>
                <p class="text-[11px] text-slate-400 mt-1">Karya: <span id="cloneSourceTeacher" class="font-semibold text-slate-600">-</span></p>
            </div>

            <div>
                <label for="custom_title" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                    Judul Baru Modul (Opsional)
                </label>
                <input type="text" name="custom_title" id="custom_title"
                       placeholder="Biarkan kosong untuk menggunakan judul asli"
                       class="w-full px-3.5 py-2.5 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-slate-800 font-medium">
            </div>

            <div>
                <label for="class_id" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                    Tugaskan ke Kelas <span class="text-rose-500">*</span>
                </label>
                <select name="class_id" id="class_id" required
                        class="w-full px-3.5 py-2.5 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-slate-800 font-semibold cursor-pointer">
                    <option value="">-- Pilih Kelas Target Pembelajaran --</option>
                    @foreach($allClasses as $cls)
                        <option value="{{ $cls->id }}">{{ $cls->grade }} {{ $cls->major_name }}</option>
                    @endforeach
                </select>
                <p class="text-[11px] text-slate-400 mt-1">Pilih rombel siswa yang akan mempelajari modul hasil salinan ini.</p>
            </div>
...
@push('scripts')
<script>
    function openCloneModal(moduleId, title, teacherName) {
        const form = document.getElementById('cloneForm');
        form.action = "{{ url('/teacher/library') }}/" + moduleId + "/clone";
        document.getElementById('cloneSourceTitle').innerText = title;
        document.getElementById('cloneSourceTeacher').innerText = teacherName;

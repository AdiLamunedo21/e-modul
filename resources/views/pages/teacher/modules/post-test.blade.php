...
{{-- ══ Header Banner ══ --}}
<div class="bg-gradient-to-r from-slate-900 via-teal-950 to-slate-900 rounded-3xl p-6 sm:p-8 text-white shadow-xl shadow-slate-950/30 mb-8 relative overflow-hidden border border-teal-500/30">
    <div class="absolute -right-10 -bottom-10 w-64 h-64 bg-teal-500/15 rounded-full blur-3xl pointer-events-none"></div>
...
<form id="post-test-form" action="{{ route('teacher.modules.post-test.update', $module) }}" method="POST">
    @csrf
    @method('PATCH')
            ...
            <div class="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-100">
                    ...
                    <label class="inline-flex items-center cursor-pointer select-none shrink-0 group">
                        <input type="checkbox" name="has_post_test" id="has_post_test_toggle" value="1"
                               class="sr-only"
                               {{ old('has_post_test', $module->has_post_test) ? 'checked' : '' }}
                               onchange="togglePostTestStatus(this.checked)">
                        <div id="post-test-toggle-track"
                             class="relative w-12 h-7 rounded-full border-2 transition-colors duration-300 ease-in-out {{ old('has_post_test', $module->has_post_test) ? 'bg-emerald-500 border-emerald-600' : 'bg-slate-200 border-slate-400 hover:border-slate-500' }}">
                            <div id="post-test-toggle-thumb"
                                 ...
                                 style="transform: translateX({{ old('has_post_test', $module->has_post_test) ? '20px' : '0px' }});">
                            </div>
                        </div>
                        <span id="toggle-status-badge" class="ml-3 text-xs font-extrabold uppercase px-2.5 py-1 rounded-full border transition-all duration-300 shrink-0 {{ old('has_post_test', $module->has_post_test) ? 'bg-emerald-100 text-emerald-800 border-emerald-300' : 'bg-slate-100 text-slate-600 border-slate-300' }}">
                            {{ old('has_post_test', $module->has_post_test) ? 'Aktif (ON)' : 'Nonaktif (OFF)' }}
                        </span>
                    </label>
                </div>
...
<script>
            ...
            return `
                <div class="option-row flex items-center gap-3 p-2.5 rounded-xl border border-slate-200 bg-slate-50/70 ${isCorrect ? 'is-correct' : ''}">
                    ...
                    <input type="text" name="questions[${index}][pilihan][${opt}]"
                           ...
                           class="flex-1 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-900 focus:border-teal-500 focus:outline-none focus:ring-1 focus:ring-teal-500 transition-all">
                </div>
            `;
        }).join('');
    ...
    function onCorrectAnswerChange(radio, index) {
        ...
        rows.forEach(row => {
            ...
            if (r && r.checked) {
                ...
                if (label) {
                    label.classList.remove('bg-slate-200', 'text-slate-700');
                    label.classList.add('bg-emerald-600', 'text-white');
                }
            } else {
                row.classList.remove('is-correct');
                if (label) {
                    label.classList.remove('bg-emerald-600', 'text-white');
                    label.classList.add('bg-slate-200', 'text-slate-700');
                }
            }
        });
    }
    ...
    function addNewQuestion(data = {}) {
        ...
        const html = getQuestionTemplate(currentCount, data);
    ...
    function renumberQuestions() {
        ...
        if (cards.length === 0) {
            const emptyCheck = document.getElementById('empty-state');
            if (!emptyCheck) {
                container.innerHTML = `
                    <div id="empty-state" class="text-center py-12 px-4 border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50/50">
                        <div class="w-14 h-14 rounded-2xl bg-teal-50 text-teal-600 flex items-center justify-center mx-auto mb-3">
    ...
    function updateSummary() {
        ...
        if (jumpContainer) {
            ...
            cards.forEach((_, idx) => {
                ...
                btn.type = 'button';
                btn.className = 'w-8 h-8 rounded-lg bg-slate-100 hover:bg-teal-600 hover:text-white text-xs font-bold text-slate-700 transition-all flex items-center justify-center';

...
<nav class="flex items-center gap-2 text-sm text-slate-500 mb-6">
    ...
    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5"/></svg>
    <span class="font-semibold text-slate-800">Bagian Inti: Materi & PPT</span>
</nav>
    ...
    <div class="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        <div>
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/15 backdrop-blur-md border border-white/20 text-xs font-bold uppercase tracking-wider text-blue-100 mb-3">
                <span>📖 Bagian Inti — Komponen 2</span>
                <span>•</span>
                <span class="text-amber-300">Opsional (Toggle)</span>
            </div>
            <h1 class="text-2xl sm:text-3xl font-extrabold tracking-tight">Notepad Editor: Materi & PPT</h1>
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-100">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-500">Jika diaktifkan, halaman Materi & Slide PPT akan disajikan pada Bagian Inti bagi siswa.</p>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-5 pt-6">
                    ...
                    <div>
                        ...
                        <div class="flex rounded-xl border border-slate-300 bg-slate-50 overflow-hidden focus-within:border-blue-500 focus-within:bg-white focus-within:ring-2 focus-within:ring-blue-500/20 transition-all shadow-sm">
                            <input type="number" name="estimasi_waktu" min="5" max="300"
                                   value="{{ old('estimasi_waktu', $data['estimasi_waktu'] ?? 45) }}"
                        ...
                        <p class="text-[11px] text-slate-400 mt-1">Perkiraan waktu baca & pemahaman.</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-100 mb-4">
                    ...
                    <div class="flex items-center gap-2">
                        <button type="button" onclick="loadSampleNotepadContent()"
                                class="px-3 py-1.5 text-xs font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl transition-all flex items-center gap-1.5">
...
<script>
    ...
    // ── TEMPLATE CONTENT ──────────────────────────────────
    function loadSampleNotepadContent() {
        if (!confirm('Muat template materi lengkap dengan tabel & format visual? Teks saat ini akan digantikan.')) return;

        document.querySelector('input[name="judul_materi"]').value = 'Kegiatan Belajar 1: Konsep & Implementasi Basis Data Relasional';
        document.querySelector('input[name="estimasi_waktu"]').value = 45;

        const sampleHtml = `
            <h2>A. Pengantar Sistem Basis Data</h2>
            <p>Dalam era teknologi informasi, sistem basis data memegang peranan krusial untuk menyimpan, mengorganisasi, dan mengamankan data perusahaan secara efisien.</p>

            <div class="callout-info">
                <p><strong>ℹ️ Informasi:</strong> Modul ini menggunakan MySQL / MariaDB yang terintegrasi di dalam lingkungan server lokal Laragon.</p>
            </div>

            <h2>B. Komponen Arsitektur Database</h2>
            <p>Sistem basis data terdiri dari beberapa komponen utama yang saling berinteraksi:</p>

            <table class="w-full border-collapse border border-slate-300 my-4 text-sm">
                <thead>
                    <tr class="bg-slate-100">
                        <th class="border border-slate-300 p-2 font-bold">Komponen</th>
                        <th class="border border-slate-300 p-2 font-bold">Fungsi Utama</th>
                        <th class="border border-slate-300 p-2 font-bold">Contoh Konkret</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td class="border border-slate-300 p-2 font-semibold">Hardware</td>
                        <td class="border border-slate-300 p-2">Media penyimpanan fisik & pemrosesan data</td>
                        <td class="border border-slate-300 p-2">Server, SSD, RAM</td>
                    </tr>
                    <tr>
                        <td class="border border-slate-300 p-2 font-semibold">DBMS</td>
                        <td class="border border-slate-300 p-2">Perangkat lunak pengelola basis data</td>
                        <td class="border border-slate-300 p-2">MySQL, PostgreSQL, Oracle</td>
                    </tr>
                    <tr>
                        <td class="border border-slate-300 p-2 font-semibold">Database Administrator</td>
                        <td class="border border-slate-300 p-2">Orang yang mengelola hak akses & keamanan</td>
                        <td class="border border-slate-300 p-2">DBA Team</td>
                    </tr>
                </tbody>
            </table>

            <h2>C. Contoh Perintah SQL Dasar (DML & DDL)</h2>
            <p>Berikut adalah contoh sintaks SQL untuk membuat tabel dan memasukkan data:</p>

            <pre class="bg-slate-900 text-slate-100 p-4 rounded-xl font-mono text-xs overflow-x-auto my-3"><code>-- 1. Membuat tabel siswa
CREATE TABLE siswa (
...
INSERT INTO siswa (nisn, nama, kelas) VALUES ('0012345678', 'Bagas Pratama', 'XI RPL');</code></pre>

            <div class="callout-tip">
                <p><strong>💡 Tips Belajar:</strong> Selalu periksa tipe data kolom dan batasan (Constraints) seperti NOT NULL atau UNIQUE sebelum mendesain tabel produksi.</p>
            </div>
        `;

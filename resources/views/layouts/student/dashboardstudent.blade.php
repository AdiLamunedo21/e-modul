            ...
            <div class="relative transform overflow-hidden rounded-3xl bg-white text-left shadow-2xl transition-all sm:w-full sm:max-w-md border border-slate-200"
                ...
                <div class="px-6 pt-6 pb-4 flex items-center justify-between border-b border-slate-100">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="text-[15px] sm:text-base font-extrabold text-slate-900">
                                Tambah & Gabung Kelas
                            </h3>
                ...
                <form action="{{ route('student.join-class') }}" method="POST">
                    ...
                    <div class="px-6 py-5 space-y-4">
                        ...
                        @if($enrolledClasses->isNotEmpty())
                            <div class="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-2">
                                <p class="text-[10px] sm:text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                                    Kelas yang Sudah Diikuti ({{ $enrolledClasses->count() }}):
                                </p>
                                <div class="flex flex-wrap gap-1.5 max-h-24 overflow-y-auto">
                                    @foreach($enrolledClasses as $enrolled)
                                        <div class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-white border border-slate-200 text-[11px] sm:text-xs font-semibold text-slate-700 shadow-2xs">
                                            <span>{{ $enrolled->full_name }}</span>
                                            <span class="font-mono font-bold text-[10px] sm:text-[11px] bg-white px-2 py-0.5 rounded-md border border-slate-200 text-emerald-700 shrink-0">
                                                {{ $enrolled->code }}
                                            </span>
                                        </div>
                                    @endforeach
                        ...
                        <div>
                            ...
                            @error('class_code')
                                <p class="text-red-500 text-[11px] sm:text-xs mt-1.5 font-medium text-center">{{ $message }}</p>
                            @enderror
                        </div>
                    </div>
                    ...
                    <div class="px-6 py-4 border-t border-slate-100 bg-slate-50/80 flex items-center justify-end gap-2.5">
                        <button @click="joinModalOpen = false" type="button" class="px-4 py-2.5 text-[11px] sm:text-xs font-bold text-slate-600 hover:bg-slate-200/70 rounded-xl transition-colors">
                            Batal
                        </button>
                        <button type="submit" class="px-5 py-2.5 text-[11px] sm:text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-500 rounded-xl shadow-md shadow-emerald-600/25 hover:shadow-lg transition-all flex items-center gap-1.5">
